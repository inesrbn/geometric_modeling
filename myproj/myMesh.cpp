...
bool myMesh::canCollapse(myHalfedge* h)
{
	...
	// VÃ©rification rapide que les faces autour de l'arÃªte sont bien des triangles
	...
	// RÃ©cupÃ©ration des sommets opposÃ©s des deux triangles adjacents
	...
	// Pour une arÃªte interne standard, les seuls sommets communs autorisÃ©s sont v3 et v4 (donc exactement 2).
	// Si le maillage possÃ¨de plus de 2 sommets communs, faire le collapse va Ã©craser des triangles non adjacents,
...
{
	// SÃ©curitÃ© absolue avant d'effectuer toute modification structurelle
	if (!canCollapse(h)) {
		return;
	}
	...
	// Calculer la position mÃ©diane et dÃ©placer le sommet survivant (v1)
	v1->point->X = 0.5 * (v1->point->X + v2->point->X);
	v1->point->Y = 0.5 * (v1->point->Y + v2->point->Y);
	v1->point->Z = 0.5 * (v1->point->Z + v2->point->Z);

	// Toutes les arÃªtes partant du sommet v2 partent maintenant de v1
	for (unsigned int i = 0; i < halfedges.size(); i++)
	{
	...
	// Nettoyage de la mÃ©moire via des fonctions lambdas utilitaires
	...
	// Suppression faces, arÃªtes supprimÃ©es et du sommet v2
	removeFace(f1);
	removeFace(f2);
...
{
	// Historique des arÃªtes refusÃ©es ou impossibles Ã  effondrer
	...
	for (int i = 0; i < iterations; i++)
	{
		...
		// Recherche de l'arÃªte la plus courte strictement valide topologiquement
		for (unsigned int j = 0; j < halfedges.size(); j++)
		{
			...
			// Ignorer si cette arÃªte a Ã©tÃ© mise sur liste noire
			bool ignore = false;
			...
			// On Ã©carte immÃ©diatement l'arÃªte si elle brise le maillage
			if (!canCollapse(h)) {
				continue;
			}
		...
		// Si le filtre a tout bloquÃ©, on arrÃªte
		if (best == NULL) {
		...
		// Si l'effacement a Ã©chouÃ©, on place l'arÃªte sur liste noire
		if (old_edge_count == new_edge_count) {
			ignored_edges.push_back(best);
			i--; // Ne compte pas comme une itÃ©ration valide
		}
	}

	// Recalcul des normales et vÃ©rification finale de la structure
	computeNormals();
